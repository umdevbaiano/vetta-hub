    ...
    <div className="container">
      <div className="infra-inner r">
        <div>
          ...
          <h2 className="st" style={{ marginBottom: 16 }}>Sistemas que<br />não podem <em>cair.</em></h2>
  ...
  <section id="ecossistema-orbital">
    <div className="container">
      ...
      <div className="orbital-stage r" data-d="3" id="orbital-stage">
        {/* rings */}
        <div className="orbit-ring" style={{ width: 420, height: 420, marginTop: -210, marginLeft: -210, top: '50%', left: '50%', position: 'absolute' }}></div>
        <div className="orbit-ring" style={{ width: 240, height: 240, marginTop: -120, marginLeft: -120, top: '50%', left: '50%', position: 'absolute', borderColor: 'rgba(0,212,255,.04)' }}></div>
        {/* center */}
    ...
    <div className="scroll-card-wrap" id="scroll-card-wrap">
      <div style={{ width: '100%', maxWidth: 860, margin: '0 auto', padding: '0 20px' }}>
        <div className="scroll-card-title r">
          ...
          <h2 className="st" style={{ textAlign: 'center', marginBottom: 8 }}>Controle real,<br /><em>visibilidade total.</em></h2>
        ...
        <div className="scroll-card-frame" id="scroll-card-frame">
          <div className="scf-bar">
            ...
            <span style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 8 }}>vetta.hub — painel de projetos</span>
          </div>
    ...
    <div className="container">
      <div className="faq-wrap">
        <div className="faq-sticky">
          ...
          <p className="ssub r" data-d="2" style={{ marginTop: 20 }}>Tem outra dúvida? Basta chamar no WhatsApp — respondemos na hora.</p>
        </div>
...
(function(){
  ...
  requestAnimationFrame(()=>{
    ...
    if(tagline) setTimeout(()=>{ tagline.classList.add('show'); }, 400);
    setTimeout(()=>{
      if(ringWrap) ringWrap.classList.add('show');
      if(lrFill) setTimeout(()=>{ lrFill.classList.add('run'); }, 50);
    }, 700);
  });
...
(function animR(){rx+=(mx-rx)*.1;ry+=(my-ry)*.1;if(curR){curR.style.left=rx+'px';curR.style.top=ry+'px';}requestAnimationFrame(animR);})();
document.querySelectorAll('a,button,.svc-card,.pr-card,.eco-layer,.proc-step,.fc,.orbit-node').forEach(el=>{
  el.addEventListener('mouseenter',()=>{if(cur)cur.classList.add('h');if(curR)curR.classList.add('h');});
  el.addEventListener('mouseleave',()=>{if(cur)cur.classList.remove('h');if(curR)curR.classList.remove('h');});
});
...
function activateNode(id){
  ...
  setTimeout(()=>{ autoRot=true; if(detail) detail.classList.remove('show'); document.querySelectorAll('.orbit-node').forEach(n=>n.classList.remove('active')); activeOrbit=null; }, 4000);
}
